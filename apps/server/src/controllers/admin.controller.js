...
const addNewLinkMappingController = (req, res) => {
  ...
  try {
    db.serialize(() => {
      db.run(
        "INSERT INTO link(slug, destination, created_at) VALUES(?, ?, ?)",
        [slug, destination, created_at],
        function (err, row) {
          if (err) {
...
const deleteLinkMappingController = (req, res) => {
  ...
  try {
    db.serialize(() => {
      db.run("DELETE FROM link WHERE slug = ?", [slug], function (err, row) {
        if (err) {
          console.log(err.message);
        }
      });
    ...
    return res.status(200).json({ slug: slug, message: "Deleted" });
  } catch (err) {
    console.log(err.message);
    return res.status(500).json({ error: err.message });
  }
